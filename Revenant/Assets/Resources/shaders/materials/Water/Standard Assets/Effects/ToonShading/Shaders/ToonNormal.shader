Shader "Toon/Lit Metal Normal" {
	...
	sampler2D _Ramp;
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			float highlight = dot(normalize(IN.lightDir), IN.wNormal); // highlight based on light direction
			float3 highlightAlbedo = (step(_HighlightOffset,highlight * nor.b) * _HiColor); //glowing highlight
			o.Albedo = c + baseAlbedo + specularAlbedo + highlightAlbedo;// result
